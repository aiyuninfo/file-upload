...
(function() {
    ...
    function fileUploadController($scope,FileUploader){
        ...
        if(ngModel){
            if(ngModel instanceof Array){
                $scope.formImages = ngModel;
            }else if(ngModel instanceof Object){
                $scope.formImages.push(ngModel);
            }else if(typeof ngModel == "string"){
                var file = {
                    fileName : ngModel.substring(ngModel.lastIndexOf("/") + 1,ngModel.lastIndexOf(".")),
                    path : ngModel
                };
        ...
        function putNgModelVal() {
            var imgList = [];
            $scope.formImages.forEach(function (val) {
                imgList.push(val);
            });
            ...
            if($scope.isSingle === "true"){
                $scope.ngModel = imgList.length > 0 ? imgList[0] : {};
            }else{
                $scope.ngModel = imgList;
            }
        }
